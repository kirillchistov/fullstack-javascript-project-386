...
| Хранилище | SQLite (`node:sqlite`, `backend/src/db.js` + `store.js`) | Персистентность для P0 (регистрация, несколько владельцев). Путь — `DATABASE_PATH` (файл или `:memory:` в тестах). Доступ к данным изолирован в `store.js`. |
...
- `Owner` — владелец календаря с публичным `slug`. Регистрация через API; при пустой БД сидируется демо-владелец.
- `EventType` — тип события («Вводный звонок») с настраиваемой длительностью 15–240 мин (кратно 15).
- `AvailabilityRule` / `Availability` — недельное расписание доступности + часовой пояс владельца.
- `Slot` — вычисляемый свободный интервал длины `durationMinutes` выбранного типа (startsAt/endsAt в UTC).
