...
|---|---|---|
| Бэкенд | Fastify (Node.js, ESM) | Валидация запросов/ответов по JSON Schema встроена в роуты — схемы берутся напрямую из скомпилированного контракта. Лёгкий, быстрый, стандартный для Hexlet-проектов. |
| БД | SQLite + Knex/Objection | Нулевая настройка для разработки и деплоя MVP; миграции через Knex. Данных мало (правила доступности + брони), реляционная модель очевидна. |
| Фронтенд | React + webpack | SPA: публичная страница бронирования и кабинет владельца. Клиент к API генерируется из `openapi.yaml`, руками HTTP-вызовы не пишутся. |
...
flowchart LR
  ...
  subgraph serverSide [Бэкенд: Fastify]
    ...
    db[(SQLite: owners, event_types, availability_rules, bookings)]
...
3. Бэкенд и фронтенд дорабатываются независимо, опираясь только на обновлённый контракт.
