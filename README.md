...
### Шаг 10 (ветка sprint11 P1)
- [x] Контракт API 0.4.0 
- [x] Busy-sync: Google OAuth или stub + ICS/URL; занятость вычитается из слотов. Apple CalDAV — нет.
- [x] Команда: организации и участники со своими /u/slug, без round-robin.
- [x] Серии + аналитика: еженедельные серии, GET /api/analytics.
- [x] Платные встречи: priceRub + YooKassa или stub /pay/stub/:id.
- [x] Freemium: Owner.plan free/pro; Pro-фичи за paywall; подписка — код pro-dev / BILLING_PRO_CODE.
- [x] Правки по review2: Тип встречи в таблице владельца (в Booking добавлено поле eventTypeName (как в GuestBooking), бэкенд отдаёт его через JOIN с event_types, в OwnerBookingsPage — колонка «Тип встречи».
- [x] Правки по review2: Гонка запросов слотов. В BookingPage каждый запрос получает монотонный requestId; setSlots вызывается только если ответ ещё актуален. При смене даты/типа или размонтировании старые ответы инвалидируются.
- [x] Тесты: Добавлен CT-тест запоздавший ответ за старую дату не перезаписывает слоты новой даты: первый запрос (сегодня) задерживается на 400 ms и отдаёт 10:00, второй (завтра) — сразу 15:00; после прихода «старого» ответа остаётся 15:00.
